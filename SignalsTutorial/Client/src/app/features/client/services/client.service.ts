...
export class ClientService {
  ...
  getClients(): Observable<Client[]> {
    return this.httpService.fetchClient().pipe(catchError(this.handleError<Client[]>('getClients', [])));
  }

  addUpdateClient(client: Client): Observable<Client> {
    return this.httpService.addClient(client).pipe(
      map((response) => {
  ...
  getLineOfBusiness(): Observable<LineOfBusiness[]> {
    return this.httpService
      .fetchLineOfBusiness()
      .pipe(catchError(this.handleError<LineOfBusiness[]>('getClients', [])));
  }
